...
Setup using Docker on **Ubuntu 20.04 LTS, 22.04 LTS, or 24.04 LTS**.
...
> Looking for native setup instead? See [setup-ubuntu-native.md](setup-ubuntu-native.md).
...
Follow the official guide for your Ubuntu version: https://docs.docker.com/engine/install/ubuntu/
...
To have Docker services restart automatically after a server reboot, ensure the Docker daemon itself is enabled, then use `restart: unless-stopped` in `docker-compose.yml` (already set by default):

```bash
sudo systemctl enable docker
```

Docker Compose services with `restart: unless-stopped` will come back up automatically when the daemon starts.
...
### Port already in use
If a port is used by a locally running service:
```bash
