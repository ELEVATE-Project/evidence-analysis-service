...
class StorageService:
    ...
    async def upload_file(
        ...
        """Upload file to configured storage and return storage URL/path."""
        if self.storage_type == "gcp":
    ...
    async def _upload_to_gcp(self, file_content: bytes, filename: str, content_type: str) -> str:
    ...
    async def _download_from_gcp(self, gcs_url: str) -> Optional[bytes]:
    ...
    async def get_file_path(self, file_url: str) -> str:
        """Get local file path (downloads cloud file into temp location if needed)."""
        ...
        temp_path = temp_dir / Path(file_url).name
        with open(temp_path, "wb") as f:
            f.write(content)
        return str(temp_path)

    async def generate_presigned_url(self, file_url: str, expiration: int = 3600) -> str:
        """Generate signed URL for cloud storage; return input for local storage."""
        if self.storage_type == "local":
            return file_url
