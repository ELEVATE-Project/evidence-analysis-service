...
("Unsupported"). Every notValidated row has blank Q&A by construction, so filtering on blank Q&A
is a strict superset of filtering on Relevance Tag == notValidated — it also catches the
Failed/Unsupported/User-Owned rows that a notValidated-only filter would miss. These rows carry no
evaluation signal, so removing them for final delivery is a separate, deliberate step rather than
...
def remove_not_validated(input_csv: str, output_csv: str) -> tuple[int, int, list[str]]:
    """Write input_csv to output_csv with rows missing either Q&A column removed.
...
if __name__ == "__main__":
    ...
    total, removed, extracted_urls = remove_not_validated(args.input_csv, args.output_csv)
    kept = total - removed
